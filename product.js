...
class ProductPage {
        ...
        this.product.variations.forEach((variation, index) => {
            ...
            variationCard.innerHTML = `
                <div class="variation-header">
                    <h4>${variation.title || `Variation ${index + 1}`}</h4>
                    ${variation.photoUrl ? `<img src="${variation.photoUrl}" class="variation-thumb" alt="${variation.attr_name}">` : ''}
                </div>
                <div class="variation-details">
                    <p><strong>Type:</strong> ${variation.attr_name}</p>
                    <p><strong>Stock:</strong> ${variation.stock} units</p>
                    <p><strong>Pieces per Unit:</strong> ${variation.piece_count}</p>
                    <p><strong>Total Pieces:</strong> ${variation.stock * variation.piece_count}</p>
                    <p class="variation-price"><strong>Price:</strong> KES ${variation.price.toLocaleString()}/unit</p>
                </div>
    ...
    selectVariation(index) {
        this.selectedVariation = this.product.variations[index];
        ...
        this.productPriceEl.textContent = `KES ${this.selectedVariation.price.toLocaleString()}`;
        ...
        showNotification(`Selected: ${this.selectedVariation.title} - ${this.selectedVariation.attr_name}`);
    }
        ...
        modal.innerHTML = `
            <div class="quantity-modal-content">
                ...
                <div class="quantity-total">
                    <p>Total: <span id="quantityTotal">KES ${this.product.price.toLocaleString()}</span></p>
                </div>
    ...
    proceedToBuyNowCheckout(quantity) {
        try {
            ...
            this.setCookie('buyNowItem', buyNowData, 1); // Expires in 1 day
            
            showNotification("Proceeding to checkout!");
            animateButton(this.buyNowBtn);
            
            // Redirect to checkout with source=buynow
